...
from app.config.config import DATA_DIR, CHUNK_SIZE, CHUNK_OVERLAP
    ...
    try:
        ...
        if not documents:
            raise CustomException(
                f"No PDF documents found in the specified DATA directory: {DATA_DIR}"
            )

        logger.info(
            f"Found {len(documents)} PDF documents in the DATA directory: {DATA_DIR}"
        )
